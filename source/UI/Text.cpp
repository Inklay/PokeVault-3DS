...
void Text::draw_lines_centered_XY(Vec2 size) {
	float height;
	float width;
...
void Text::draw_lines_centered_Y(int y) {
	float height;
	C2D_TextGetDimensions(&m_text, 0.5f, 0.5f, 0, &height);
	int text_y = y / 2 - height / 2 + m_pos.y;
	C2D_DrawText(&m_text, C2D_WithColor, m_pos.x, text_y, m_pos.z + 1, 0.5f, 0.5f, m_color);
}

void Text::draw_lines_centered_X(int x) {
	float width;
	C2D_TextGetDimensions(&m_text, 0.5f, 0.5f, &width, 0);
