...
void Text::DrawLines(void) {
	C2D_DrawText(&m_text, C2D_WithColor, m_pos.x, m_pos.y, m_pos.z, 0.5f, 0.5f, m_color);
}

void Text::DrawLinesCenteredXY(Vec2 size, float font_size) {
	float height;
	float width;
...
void Text::DrawLinesCenteredY(int y, float font_size) {
	float height;
...
void Text::DrawLinesCenteredX(int x, float font_size) {
	float width;
